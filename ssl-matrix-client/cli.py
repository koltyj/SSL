...
from .protocol import PROTOCOL_NAMES, PROTOCOL_BY_NAME, PORT
...
class SSLMatrixCLI(cmd.Cmd):
    ...
    def do_status(self, arg):
        ...
        if not self._connected:
            ...
            return
        d = self.client.state.desk
        print(f"Console:    {d.product_name} '{d.console_name}'")
        print(f"Firmware:   {d.firmware}")
        print(f"Serial:     {d.serial}")
    ...
    def do_transportlock(self, arg):
        ...
        self.client.send(build_set_transport_lock(
    ...
    def do_motors(self, arg):
        """Get or set motors off. Usage: motors [on|off]"""
        if not self._require_connected():
            return
        if not arg.strip():
            m = self.client.state.motors_off
            print(f"Motors off: {'on' if m else 'off'}")
            return
        val = arg.strip().lower()
        if val == "on":
            self.client.set_motors_off(1)
        elif val == "off":
            self.client.set_motors_off(0)
        else:
    ...
    def do_projects(self, arg):
        ...
        time.sleep(0.5)
        entries = self.client.state.directory
        if not entries:
    ...
    def do_xpatch_setup(self, arg):
        ...
        for c in chans:
            ...
            print(f"{c.number:3d}  {c.device_name:16s} {c.dest_name:16s} {i10:5s} {o10:6s} {c.mode:4d}")
    ...
    def do_state(self, arg):
        ...
        d = {
            "desk": {
                ...
                "heartbeat_age": round(s.desk.heartbeat_age, 1),
            },
            "channels": {ch.number: ch.name for ch in s.channels if ch.name},
            "daw_layers": [
                {"layer": dl.number, "protocol": PROTOCOL_NAMES.get(dl.protocol, "?"),
                 "profile": dl.profile_name}
                for dl in s.daw_layers
            ],
            "profiles": [
                {"name": p.name, "protocol": PROTOCOL_NAMES.get(p.protocol, "?"),
                 "read_only": p.read_only, "in_use": p.in_use}
                for p in s.profiles
            ],
            "devices": [
                {"num": d.number, "name": d.name, "assigned": bool(d.is_assigned),
                 "stereo": bool(d.is_stereo)}
                for d in s.devices if d.name
            ],
            "chains": [
                {"num": c.number, "name": c.name, "assigned": bool(c.is_assigned),
                 "elements": [{"idx": i, "name": n} for i, n in c.elements]}
                for c in s.chains
            ],
            "channel_inserts": [
                {"channel": ci.channel, "chain": ci.chain_name,
                 "inserts": ci.inserts, "stereo": bool(ci.has_stereo)}
            ...
            "xpatch_presets": [
                {"index": p.index, "name": p.name, "used": p.used}
                for p in s.xpatch.presets
            ],
            ...
            "title": s.title_name,
            "directory": [
                {"name": e.name, "is_dir": e.is_dir, "date": e.date_str,
                 "time": e.time_str, "size": e.size}
...
def main():
    ...
    parser.add_argument("command", nargs="*",
                        help="One-shot command (e.g. 'channels', 'layers')")
